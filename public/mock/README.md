...
| `pill-white.jpg`     | 흰색 동그란 약 (a single white round pill)         | MedicationPhotoPage / DosePage pill list (흰색 동그란 약)                    |
| `pill-yellow.jpg`    | 노란색 긴 약 (a single yellow oval pill)           | MedicationPhotoPage / DosePage pill list (노란색 긴 약)                      |
| `pill-blue.jpg`      | 파란색 캡슐 (a single blue capsule)                | MedicationPhotoPage / DosePage pill list (파란색 캡슐)                       |
| `taken.jpg`          | 빈 봉지 + 물컵 (empty pouch next to a water glass) | Gallery / Dashboard (완료된 dose photo)                                     |

Notes:
- Any image format the browser can render works; `.jpg` is what the paths expect.
